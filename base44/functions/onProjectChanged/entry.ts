  ...
  try {
    ...
    if (event.type === 'update' && actorIsAssignee) {
      ...
      if (data.status === 'completed' && old_data?.status !== 'completed') {
        ...
        if (assignerEmail) {
          const subject = `[FSQA] Project completed: ${data.name}`;
          const body = [
            `Hi,`,
            '',
            `<strong>${data.fsqa_assignee}</strong> has marked the following project as <strong>completed</strong>.`,
            '',
      ...
      return Response.json({ skipped: 'actor is the assignee and project not completed' });
    }
