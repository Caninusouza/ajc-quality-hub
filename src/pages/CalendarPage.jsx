...
export default function CalendarPage() {
  ...
  // Build events map: date string → array of events
  const eventsMap = {};
  ...
  return (
    <div>
      <PageHeader title="FSQA Calendar" subtitle="Tasks and project due dates at a glance" />
                ...
                <div className="space-y-0.5">
                  {dayEvents.slice(0, 3).map((ev, j) => (
                    <div key={j} className={cn('text-[10px] px-1 py-0.5 rounded border truncate', getItemColor(ev))}>
                      {ev._stepLabel ? `📍 ${ev._stepLabel}` : ev._type === 'task' ? `✓ ${ev.title}` : `◆ ${ev.name}`}
                    </div>
                  ))}
                  {dayEvents.length > 3 && (
          ...
          {selectedEvents.length === 0 ? (
            ...
            <div className="space-y-2">
              {selectedEvents.map((ev, i) => {
                ...
                return (
                  <div key={i} className={cn('flex items-start gap-3 p-3 rounded-lg border text-sm', getItemColor(ev))}>
                    <span className="shrink-0 font-bold">{ev._stepLabel ? '📍' : ev._type === 'task' ? '✓' : '◆'}</span>
                    <div className="min-w-0 flex-1">
                      <p className="font-medium truncate">
                        {ev._stepLabel ? `${ev.name} — ${ev._stepLabel}` : ev.title || ev.name}
                      </p>
                      <p className="text-xs opacity-70 mt-0.5">
                        {ev._type === 'task' ? 'Task' : ev._stepLabel ? 'Project Timeline' : 'Project'}
                        {rep ? ` · ${rep}` : ''}
